...
13. Core constructs the exact declared ClaimKey. The M0 ClaimPolicy may validate/reject it and choose `exclusive|coexisting`; it has no `advise` method and may not transform identity. Optional mechanical policy advice requires a later additive API when M2/M4 executes it. Default semantics are declared-key and exclusive. Generic same-value/different-key advice is later M1.5 versioned core mechanics, not policy advice, and does not reconcile across keys.
