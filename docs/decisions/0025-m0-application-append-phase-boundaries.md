...
References are extracted only from the canonical draft and read sequentially in the contract's field/index order. A present value is decoded and its returned id is checked against the requested id before family classification. Missing and wrong-kind results accumulate in traversal order; wrong kind includes disagreement between the complete reference id's prefix and the returned record family, even for Relation fields that otherwise allow every family. A `get` throw, malformed returned record, or returned id different from the requested id stops the reference phase and becomes a fresh `REFERENCE_CHECK_FAILED` with no foreign issues or cause. SourceRefs produce no store reads.
