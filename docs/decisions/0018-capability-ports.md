...
- Production kernel code must not bypass these ports with `Date.now()`, zero-argument `new Date()`, or `Math.random()`. This belongs in the existing lightweight workspace structural guard now; it does **not** need to wait for dependency-cruiser issue #18, because import-graph tooling cannot see calls with no import.
