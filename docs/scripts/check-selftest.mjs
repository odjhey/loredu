...
const CASES = [
  ...
  },
  {
    ...
    script: 'check-docs.mjs',
    expect: 'frontmatter',
    mutate: (root) => edit(root, A_DOC, (t) => t.replace(/^created_at: (.*)$/m, 'created_at: $1\nupdated_at: $1')),
  },
  {
  ...
  },
  {
    ...
    expect: 'orphan',
    mutate: (root) =>
      write(root, 'docs/architecture/floating.md', '---\nname: floating\ndescription: "Nothing links here."\ntype: reference\n---\n\n# Floating\n'),
  },
  {
  ...
  },
  {
    ...
    mutate: (root) =>
      write(root, 'tests/records/entry.test.ts', 'import { test, expect } from "bun:test"\n\n// @covers T01\ntest("entry accepted", () => { expect(1).toBe(1) })\n'),
  },
  {
    name: 'status file invents an unknown T-number',
    script: 'check-catalog.mjs',
    expect: 'unknown',
    mutate: (root) => editJson(root, STATUS, (d) => { d.deferred.T99 = { milestone: 'M9', reason: 'invented' } }),
  },
  {
    ...
    mutate: (root) => {
      ...
      write(root, 'tests/records/entry.test.ts', 'import { test } from "bun:test"\n\n// @covers T01\ntest("entry accepted", () => {})\n')
    },
  },
  {
  ...
  },
  {
    ...
    script: 'check-catalog.mjs',
    expect: 'status',
    mutate: (root) => editJson(root, STATUS, (d) => { d.deferred.T01 = { reason: 'no milestone given' } }),
  },
]
...
for (const testCase of CASES) {
  ...
  try {
    ...
    if (testCase.expect === null) {
      ...
      console.error(`✗ ${testCase.name}: gate did not fire — expected rule \`${testCase.expect}\`, got [${rules.join(', ') || 'none'}]`)
      failures++
