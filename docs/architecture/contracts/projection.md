...
Core first builds a full recorded-visible record index after applying `as_of`, before scope selection. Resolution reference eligibility uses that full index: every target and optional replacement must resolve to a matching lower-position record. A missing or forward reference, including one excluded by `as_of`, invalidates the whole Resolution for projection even when that reference is outside the requested scope or exact key.

Separately, scope selection builds each key's **applicable Claim set** before any preference tier: the nonempty set of its selected Claims whose validity interval contains the resolved valid-time point. No Resolution, Relation, policy result, or out-of-scope Claim may add a Claim or key to output. A key with an empty set has no Current Knowledge item.

A recorded-visible Resolution that passes full-index reference validation is projection-eligible only when it is valid-time-effective. For one applicable Claim set, it is **complete** only when its direct Claim targets include every member of that set. Relation, nonapplicable, out-of-scope, and cross-key targets do not provide coverage. Additional lower-position recorded-visible targets outside the selected group neither invalidate the Resolution nor broaden output, ordering, counts, or policy-advice Claim context; under the stricter advice-context rule, they exclude that Resolution from advice context. Decision application is exact:
