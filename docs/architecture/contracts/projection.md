...
The callback result is descriptor-checked as a normal Array with its standard own data `length` descriptor. Core validates that own length before inspecting any indexed value. A length above 200 produces a fresh `VALIDATION_FAILED` before density or element validation, advisory sorting or counting, or pagination; the operation returns no partial result. At a length of 200 or less, every index from zero through length minus one must be an own data property and no hole, accessor, custom prototype, or excess property is accepted. Each element must then be a closed plain object: `code` is an identifier-safe token, `claims` contains one or two distinct ids from the input Claim array, and `details` is a closed canonical JsonObject. Core orders the referenced handles by position and orders advisories by code, then referenced positions, preserving callback order for a remaining tie; an exact duplicate output is `VALIDATION_FAILED`, not silently coalesced. Throwing or malformed output fails the operation with a fresh `VALIDATION_FAILED`, no partial result, and no foreign details. The maximum of 200 is a protocol literal and adds no public constant or export.
...
- `prefer` — requires its replacement to be a directly targeted member of the applicable Claim set; that value is preferred while alternatives remain historical;
- `supersede` — has the same applicable replacement requirement; that value is preferred and the other applicable targets are marked superseded in history;
...
Counts cover the full pinned query, not the page. A disputed item emits corrective top-level advice in this order: its exact-key Claim list, then show for each exposed value representative. Coexisting items and policy advisories are non-blocking and add no corrective top-level advice. Continuation is last.
