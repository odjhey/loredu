...
Ranking quality beyond deterministic baseline order remains an M4 consumer judgment. Embeddings, lexical indexes, graph traversal, model reranking, source crawling, token counting, automatic Resolution, and derived-record persistence are not required. A consumer may perform those behind a versioned Ranker without changing core contracts. T41–T45/T75 are implemented by M3-L application evidence; T40 remains deferred until M3-E supplies its required compiled rendering and exit evidence.
