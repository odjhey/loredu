...
A knowledge summary deliberately omits raw values and canonical records. Its bounded `summary` provides compact key/state/value context, while handles and the exact-key `claims` affordance lead to complete values, Claim history, evidence, Entries, and terminal SourceRefs. `claim_count` is the full number of contributing applicable Claims across all surviving values, not the number of representatives. `value_count`, history, and evidence retain the M2 meanings. Retracted knowledge is omitted from default sections. Superseded or nonapplicable Claims in an included exact group remain reachable by following its representative history and exact-key Claim disclosure.

For every included knowledge occurrence, representative authority is the corresponding M2 `CurrentKnowledgeItem` produced from the same pinned prefix, normalized scope, resolved valid point, core/ClaimPolicy identity, and reconciliation result, after the whole group passes the M3 corpus filter. Core defines exactly:

```ts
representatives = currentKnowledgeItem.values.map(
  (currentValue) => currentValue.representative,
)
```

Working Lore copies those `RecordHandle`s in exact M2 exposed-value order into a newly detached, recursively frozen tuple. It performs no independent selection, sorting, filtering, replacement, confidence/evidence or latest-record preference, Ranker influence, section-specific choice, corpus-per-value choice, or deduplication. M2 alone orders surviving distinct values by earliest surviving Claim position and chooses each exposed value's representative as the selected Resolution replacement or otherwise the earliest surviving Claim for that value. Any apparent tie is already resolved by M2's exact stream-position/value rules; M3 adds no tie-breaker.

A valid included summary has exactly one or two representatives. M2 `retracted` items have zero values and are omitted entirely: no item, tuple, Ranker candidate, conflict advice, or cursor occurrence exists. Preferred and one-value disputed groups copy one handle. Coexisting or disputed groups copy M2's first one or two exposed handles; when full `value_count` is three or more, only the first two M2 values are exposed, so M2 order A/B/C yields `[A,B]`, never `[A,C]` or `[B,C]`. Canonically equal Claims remain one M2 value and one representative. A Claim has one value, so valid M2 output cannot expose one Claim handle for two distinct values; Working Lore nevertheless applies the literal map rather than deduplicating. The key's `anchor_claim` remains the earliest contributing applicable Claim for disclosure; it may differ from a value representative and never replaces or reorders the tuple.
...
Core first derives the complete, unpaginated M2 knowledge stream for the normalized scope and resolved valid point, then applies the corpus evidence filter above to each whole knowledge group. It uses ClaimPolicy validation and semantics but does not invoke the optional projection-wide `advise` callback: Working Lore candidates are Claim-backed knowledge, while policy details remain available through `current`. It never appends an inferred record. One knowledge group may intentionally occur in more than one section; each occurrence is a budgeted item and carries the same copied representative tuple.
...
ADR 0030's exact Working Lore controls narrowly supersede ADR 0009's generic `--limit` requirement for this operation only; its pinned-cursor, explicit-count, no-silent-truncation, deterministic-ordering, and disclosure rules remain. Default budgets are `max_items:40` and `max_chars:12000`. Accepted `max_items` values are safe integers from 1 through 200. Accepted `max_chars` values are safe integers from 512 through 1,000,000. A summary is at most 512 Unicode scalar values, so any accepted character budget can advance a nonempty continuation.
...
Truncation is never silent. For each returned conflict item, top-level corrective advice is its exact-key `claims.list` followed by show for each representative in tuple order; disputed M2 order A/B/C therefore emits claims, show A, show B, with C reachable through the anchored list. Items follow packet rank order and the general structural deduplication rule keeps the first affordance. Deduplication may suppress a later duplicate top-level show across returned conflict occurrences but never mutates an item's tuple. Coexisting occurrences emit no corrective top-level advice. Top-level advice then carries one `continue/lore.read` affordance for each truncated section in display order. Continuation params are exactly `{cursor}` plus `max_items` and/or `max_chars` when their effective values differ from defaults. Callers may supply new accepted budgets with the cursor.
...
Continuation consumes no Clock. Appended suffix records remain excluded by the pinned prefix. No derived item or full permutation is persisted or otherwise stored; beyond the shared cursor version, operation/query, Basis, pinned-head anchor, valid point, and `computed_at` metadata above, each opaque cursor carries only the fixed algorithm/digest/count, one bound section, and one bounded resume identity as ranking state. Representative ids do not enter permutation digest or cursor occurrence identity. Rebuilding the pinned M2 item reproduces each tuple; representative ids and order are semantic packet content, and any drift at equal Basis is replay failure rather than a new digest. Ranker, section, accepted budgets, and continuation cannot alter a tuple. Appends during a chain create no duplicates or skips; a fresh cursorless call sees the new head. Digest bytes and cursor spelling remain outside semantic replay equality.
