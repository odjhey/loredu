...
- Scope is a flat unordered token-to-token map. Absent and `{}` are identical. Claim-key equality compares its pair set independent of insertion order. Existing canonical records permit any finite pair count; M3 does not add a shared decoder maximum. Working Lore keeps the complete Scope for internal identity and its one Basis query but uses a fixed-cardinality preview in ranked candidates and packet structures.
