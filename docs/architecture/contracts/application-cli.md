...
`--body -` reads stdin to EOF as UTF-8 and passes the decoded text unchanged: no trimming or newline insertion/removal. A leading UTF-8 BOM is preserved as the U+FEFF body character rather than consumed as a signature. Invalid UTF-8 is a usage/validation failure. A terminal may supply `--body <text>` directly. Stdin is not read for another spelling or after an earlier error.
