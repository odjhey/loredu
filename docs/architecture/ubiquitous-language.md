...
| **Working Lore** | Bounded, ranked projection prepared for one activity. Its compact sections carry full counts, Basis-pinned continuations, and handles for deeper disclosure. |
...
| **Ranker** | Versioned Working Lore ordering port. Core owns section membership, counts, budgets, and item construction; a Ranker returns only a validated complete permutation of candidate occurrences. |
...
| **Basis** | Exactly stream position, structural ruleset identity, and canonical JSON query. Current Knowledge always records its resolved canonical `valid_at`; Working Lore additionally identifies its Ranker. `computed_at` is outside Basis and equality. |
