...
Every derived view is **bounded and versioned** ([decisions 0006](../decisions/0006-explicit-version-basis.md) and [0027](../decisions/0027-m2-reconciliation-projection-contract.md)): stamped with the basis (stream position, ruleset, query) it was computed from, so consumers cache safely, detect staleness by comparison, and reproduce its contract-defined semantic content deterministically. Storage may grow without bound; a caller's context never has to.
...
- bounded Working Lore, deterministic core-owned section membership/budgeting, M2-authoritative value-representative tuples, fixed-cardinality Scope/key descriptors, permutation-bound continuation integrity, and anchored progressive disclosure;
- storage/application ports required to preserve those semantics, including the capability ports the kernel is assembled with (`Clock`, `RandomSource` — [clock and identity](./contracts/clock-and-identity.md)) and the versioned ordering-only `Ranker` used by Working Lore.
...
`Ranker` follows the same application-port pattern as `Extractor` and `Resolver`, but its M3 authority is deliberately narrower: core constructs and classifies every Working Lore occurrence through bounded descriptors, and the Ranker returns only a complete ordering permutation. Core binds the validated permutation's pure-SHA-256 digest and exact occurrence resume identity into each section cursor, so a same-version ordering change fails closed rather than duplicating or skipping. Core ships a deterministic baseline; a consumer may plug in lexical search, embeddings, or model reranking without the core knowing. Ranker identity participates only in Working Lore Basis, so unrelated M0–M2 responses do not drift.

The domain and application layers must not depend on a CLI, agent harness, model SDK, crawler, or persistence vendor. Provider-native types stop at adapters. M0's RecordStore boundary is intentionally only typed `append(PersistedRecord) → StreamPosition` and `get(RecordId) → PersistedRecord|undefined`; application append owns validation, references, stamping, and the `{record, position}` result. M1 additively closes atomic snapshot `scan` with a kind-only provider filter, snapshot-bounded incremental `stream`, `head`, and reusable conformance in the [store contract](./contracts/store.md). M1.5 composes claim filters, exact-key feedback, health, Basis-pinned cursors, and semantic affordances above that port; the CLI only parses/renders them and supplies host Clock/secure RandomSource implementations at its composition root, while its exported composition seam may accept an embedding consumer's existing ClaimPolicy and host ports without changing `lor` grammar ([application and CLI contract](./contracts/application-cli.md), [ADR 0029](../decisions/0029-cli-composition-seam.md)). M2 additively derives Current Knowledge from one atomic snapshot under the active structural core/policy ruleset: policy may advise but cannot merge keys or choose truth, while `computed_at` stays outside semantic Basis ([projection contract](./contracts/projection.md)). M3 additively prepares compact Working Lore through `lore(query)`: core owns exact sections, summaries, literal M2-ordered representative copying, fixed-cardinality descriptors, counts, budgets, digest-bound cursors, and anchored disclosure; a versioned Ranker can only reorder frozen occurrences and cannot alter handles inside an occurrence ([Working Lore contract](./contracts/working-lore.md), [ADR 0030](../decisions/0030-working-lore-ranker-contract.md)). Markdown bytes, roots, locks, and fsync stay in the [plain-file provider contract](./contracts/plain-file-store.md), outside the kernel/domain boundary.
