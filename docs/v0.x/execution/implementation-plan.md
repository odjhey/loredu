...
- list items exposing one own-record handle and reference-free summaries, with full recursively rendered reference handles deferred to `show`; rendered actions preserve explicit store selection, invalid-reference diagnostics and SourceRefs are terminal, and corrective/continuation affordances are deterministic;
