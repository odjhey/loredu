...
The M1.5 exit is implemented. Executable `@covers` annotations and [`catalog-status.json`](./catalog-status.json) own current row-level accounting; M2-P delivered public Current Knowledge, M2-E delivered the compiled M2 scenario exit, M3-L delivered application-level Working Lore and section continuation, and M3-E delivered their CLI rendering and journeys.

Exit: an agent given only the binary and `lor skill` completes journeys 0, 1, 2, 3, 3b, 4, 5, 6, 7, 8, and 9 on a fresh store, follows pagination/disclosure commands, records manual judgment, and ends with `lor status --check` passing. Acceptance scenario A's records, manual relations, Current Knowledge, temporal replay, reconciliation comparison, compiled Working Lore, and revalidation assertions are executable.

## M2 — Reconciliation and projection

Implement [decision 0027](../../decisions/0027-m2-reconciliation-projection-contract.md) and the exact [projection contract](../../architecture/contracts/projection.md). The staged delivery is complete: M2-R landed the bounded internal reconciliation foundation plus additive Claim feedback and overlap-aware status health; M2-P added public Current Knowledge, temporal projection, summaries/evidence, cursor, and deterministic rebuild/invalidation wiring; M2-E added deterministic compiled scenarios A/B/C and the M2 exit evidence. M3-L and M3-E have since completed Working Lore and its compiled surface without changing M2.
...
M3 is implemented under [decision 0030](../../decisions/0030-working-lore-ranker-contract.md) and the exact [Working Lore contract](../../architecture/contracts/working-lore.md). M3-L's executable public-API evidence owns T41–T45/T75; M3-E owns T40, CLI wiring/rendering, the skill revision, and compiled journeys T54/T56:
