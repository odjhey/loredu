...
- the adopted five-family record contract: all five kinds use optional draft `sources: SourceRef[]` persisted as `[]`; drafts omit application-owned `schema`, `id`, and `recorded_at`; absent draft `scope`/`metadata` persist as `{}`; Relations are directed binary `relation_type` + ordered `{id, kind}` `from`/`to` endpoints; Verification uses non-empty Claim targets and a distinct non-empty `{source, snapshot?}` basis; collection, text, timestamp, interval, and empty M0 `loredu.*` registry rules are exact in the [record contract](../../architecture/contracts/records.md) and [ADR 0021](../../decisions/0021-m0-record-contract-closure.md); serialization/replay round-trip evidence is deliberately the M1 adapter concern (T06);
...
Exit: through public kernel APIs, drafts can be validated and appended into the test-only in-memory seam; returned records have the fixed application-owned schema, kernel-assigned IDs, explicit default maps/arrays, and `recorded_at`; malformed fields, timestamps, intervals, reserved metadata, and references fail with actionable errors; the default ClaimPolicy reproduces the declared-key/exclusive behavior; deterministic capability substitutes reproduce the same first stamped record across fresh assemblies while repeated appends consume fresh entropy; attempts to introduce environment-specific or ambient time/randomness kernel APIs fail the configured boundary/type checks. This exit does not claim durable-provider, filesystem, or codec behavior; T15 is an M0 test-seam row and T06 remains M1.
...
- portable logical RecordStore cases run against both the in-memory reference store and `PlainFileStore`; provider/durability conformance (layout, locking, atomic visibility, fsync, crash behavior, and cross-instance replay) runs against `PlainFileStore` and future durable adapters only, never as evidence from the in-memory seam.
