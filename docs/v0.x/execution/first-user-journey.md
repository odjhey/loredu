...
| T64 | bounded application status counts every absent/forward reference and unresolved exclusive conflict set: only overlapping-validity/different-value pair endpoints count and require Resolution coverage; purely disjoint succession consumes no Clock and never blocks/reopens health, while malformed canonical files fail as store corruption | journey 3b, ADR 0026/0027 |
