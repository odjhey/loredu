...
M2-R rows **T20–T23 and T86** cover the executable deterministic engine
...
the 200/201 advisory boundary. M2-P rows **T24–T30** drive the published `current`
API and cover complete Resolution integration, all four temporal modes, bounded
history/evidence, semantic replay, advice/combined-stream continuation, Basis, and
invalidation. T86 uses the M0 structural JSON equality primitive but remains M2 because
this is its first use in pair classification. M1.5 T68 continues to own the generic
cross-key hint as versioned core mechanics, not policy advice.
...
forbids a public reconciliation runtime function; M2-P tests use only published package
exports and package entrypoint/export audits prove that the implementation seam did not
become API.
